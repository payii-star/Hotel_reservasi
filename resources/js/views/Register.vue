...
async function submit() {
  ...
  try {
    ...
    errorMsg.value = errors ? Object.values(errors)[0][0] : (e.response?.data?.message || 'Gagal mendaftar, coba lagi.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="max-w-md mx-auto px-6 pt-28 pb-20">
    <h1 class="font-display text-3xl mb-2 text-ink">Daftar Akun</h1>
    <p class="text-ink/60 mb-8">Buat akun untuk mulai memesan kamar di Grand Nusantara Hotel.</p>
    ...
    <form @submit.prevent="submit" class="space-y-4">
      <div>
        ...
        <input v-model="name" type="text" required class="w-full border border-mist/40 rounded-lg px-3 py-2" />
      </div>
      <div>
      ...
      <div>
        <label class="block text-sm text-ink/60 mb-1">Konfirmasi Password</label>
        <div class="relative">
          <input
    ...
    <p class="text-sm text-ink/60 mt-6 text-center">
      Sudah punya akun?
      <RouterLink :to="{ name: 'guest-login', query: route.query }" class="text-pine hover:underline">Masuk di sini</RouterLink>
    </p>
  </div>
</template>